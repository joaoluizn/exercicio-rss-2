...
class SQLiteRSSHelper private constructor(private var c: Context) : SQLiteOpenHelper(c, DATABASE_NAME, null, DB_VERSION) {
    val items: Cursor?
...
//    Implementaçao utilizando funções de escrita e leitura do banco
    ...
    fun insertItem(title: String, pubDate: String, description: String, link: String): Long {
        ...
        return this.writableDatabase.insert(DATABASE_TABLE, null, values)
    }

    @Throws(SQLException::class)
    fun getItemRSS(link: String): ItemRSS? {
        val cur: Cursor = this.readableDatabase.query(DATABASE_TABLE, columns, "$ITEM_LINK=\'$link\'", null, null, null, null)
        return if(cur.count == 0){
            Log.d("SQL", "Nenhum resultado encontrado no Banco")
            null
        }else{
            cur.moveToFirst()
            var title = cur.getString(cur.getColumnIndexOrThrow(ITEM_TITLE))
            var pubDate = cur.getString(cur.getColumnIndexOrThrow(ITEM_DATE))
            var description = cur.getString(cur.getColumnIndexOrThrow(ITEM_DESC))
            var link = cur.getString(cur.getColumnIndexOrThrow(ITEM_LINK))
            var readStatus = cur.getInt(cur.getColumnIndexOrThrow(ITEM_UNREAD)) != 0
            Log.d("SQL", "Resultado encontrado, retornando Valor")
            ItemRSS(title, pubDate, description, link, readStatus)
        }
    ...
    fun markAsRead(link: String): Boolean {
        ...
        this.writableDatabase.update(DATABASE_TABLE, cv, "$ITEM_LINK=\'$link\'", null)
        return true
    }
